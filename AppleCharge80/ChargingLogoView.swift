    ...
    var body: some View {
        TimelineView(
            ...
            ZStack {
                ...
                // ---------------------------------------------------------
                // Поток мелких листиков и цветков.
                // Ветки, стебли, пыль и twig отсутствуют.
                // ---------------------------------------------------------
                FormationParticles(
                    progress: p,
                    time: time
        ...
        .onChange(of: isCharging) { _, charging in
            if charging {
                // Новое подключение — начинаем с чистого листа.
                logoOpacity = 0.0
                withAnimation(
                ...
                // Отключение кабеля — плавное растворение.
                withAnimation(
...
private struct AppleSectorStack: View {
    ...
    var body: some View {
        ...
        // Последовательное заполнение шести секторов.
        let scaled = progress * Double(count)
        let completed = min(
            count,
            Int(floor(scaled))
        )
...
private struct AppleSector: View {
    ...
    let bottom: CGFloat
    var body: some View {
        let amount = amountForSector()
        if amount > 0.0 {
            let fillHeight = max(
                0.5,
                bandHeight * CGFloat(amount)
            )
            let bottomY =
                bottom -
            ...
            let centerY =
                bottomY -
                fillHeight / 2.0
            // Колышется только активная граница.
            let active =
                index == completed &&
                completed < count
            let waveAmplitude =
                active
                ? max(2.0, bandHeight * 0.075)
                : 0.0
            let phase =
                time * 0.72 +
                Double(index) * 1.83
    ...
    // Очень мягкое изменение цвета.
    // Никакого белого блика или свечения.
...
private struct AppleLeafReveal: View {
    ...
    var body: some View {
        ...
        AppleLeafShape()
            ...
            .scaleEffect(
                x: 0.78 + 0.22 * leafProgress,
                y: 0.78 + 0.22 * leafProgress,
                anchor: .bottom
            )
    }
}
...
private struct FormationParticles: View {
    let progress: Double
    let time: TimeInterval
    private let particles = FormationParticle.all
    var body: some View {
        Canvas { context, size in
            guard progress < 0.985 else {
                // После формирования верхнего листа поток
                // полностью прекращается.
                return
            }
            let sectorCount = 6
            // Определяем текущий формируемый сектор.
            let scaled =
            ...
            // Сколько уже заполнено текущего сектора.
            let sectorFraction =
                scaled -
                Double(currentSector)
            for particle in particles {
                // Частицы идут постоянно, но умеренно плотно.
                // Каждая имеет собственный цикл.
                let cycle =
                    particle.duration
                let rawPhase =
                    (
                ...
                // ---------------------------------------------------------
                // Начальная точка:
                // строго нижняя центральная область.
                // ---------------------------------------------------------
                let startX =
                    size.width * 0.5 +
                ...
                let x =
                    ...
                    secondarySway
                // Нелинейное движение:
                // сначала достаточно быстро,
                // затем мягкое торможение перед поглощением.
                let yProgress =
                    easeOutCubic(phase)
                let y =
                    startY +
                ...
                // ---------------------------------------------------------
                // Поглощение.
                // Последняя часть траектории становится прозрачной,
                // создавая ощущение, что частица входит в сектор.
                // ---------------------------------------------------------
                let absorptionStart = 0.78
                let absorption: Double
                if phase < absorptionStart {
                    absorption = 1.0
                } else {
                    absorption =
                        max(
                ...
                // Не все частицы должны быть одновременно видны.
                // Это даёт плотный, но не чрезмерный поток.
                let alpha =
                    particle.opacity *
                    absorption
...
private struct FormationParticle {
    ...
    static let all: [FormationParticle] = {
        let palette: [Color] = [
            Color(red: 0.38, green: 0.78, blue: 0.26),
            Color(red: 0.30, green: 0.68, blue: 0.92),
            Color(red: 0.98, green: 0.72, blue: 0.18),
            Color(red: 0.98, green: 0.32, blue: 0.22),
            Color(red: 0.90, green: 0.30, blue: 0.62),
            Color(red: 0.62, green: 0.38, blue: 0.92)
        ]
        // 42 частиц:
        // достаточно плотный поток, но без ощущения "роя".
        return (0..<42).map { i in
            ...
            return FormationParticle(
                kind: kind,
                // Поток начинается возле центра,
                // но имеет небольшое естественное рассеивание.
                startOffset:
                    CGFloat(seed - 0.5) *
                    34.0,
                duration:
                ...
                size:
                    kind == .flower
                    ? 4.2 + CGFloat(seed) * 3.2
                    : 4.0 + CGFloat(seed) * 4.0,
                opacity:
                    0.48 +
                    seed * 0.38,
                color:
